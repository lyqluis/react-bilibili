...
export const addSource = (
	...
	const initAppendBuffer = (e) => {
		const sourceBuffer = e.target
		sourceBuffer.removeEventListener("updateend", initAppendBuffer)

		bufferSegmentIndex = 20
		sourceBuffer.addEventListener("updateend", nextAppendBuffer)
		addNextMedia(mediaUrl, bufferSegmentIndex, sidx)
	}

	const nextAppendBuffer = (e) => {
		const sourceBuffer = e.target
	...
	mediaSourceBuffer.addEventListener("updateend", initAppendBuffer)
