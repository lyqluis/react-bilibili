import { deepClone, parseDate } from "./global"

export const logoSvg =
	"https://s1.hdslb.com/bfs/static/neul-project-flow/h5/920a564690a2a8b50187e8912ea25b8de3f8d215/assets/picture-loading.dec159c3.svg"
...
export const formateFilter = (rawFilter, defaultFilter) => {
	...
	for (const key in res) {
		if (rawFilter[key]) {
			res[key] =
				typeof defaultFilter[key] === "number"
					? parseInt(rawFilter[key])
					: rawFilter[key]
		}
	}
	// field
...
export const couponUseDuration = (coupon) => {
	const { useEffectDays, useStartTime, useEndTime } = coupon
	if (useEffectDays) return `领取后${useEffectDays}天内有效`
	const options = {
		connector: ".",
		needTime: false,
		needYear: true,
	...
	return `${parseDate(useStartTime, options)}-${parseDate(useEndTime, options)}`
}
